...
WHERE u.deleted_at IS NULL
  AND (?1 IS NULL OR u.status = ?1)
  AND (?2 IS NULL OR u.platform_role = ?2)
  AND (?3 IS NULL
       OR instr(lower(u.username), ?3) > 0
       OR instr(lower(u.email), ?3) > 0)
`

type CountAdminUsersParams struct {
	Status       interface{}
	PlatformRole interface{}
	Q            interface{}
}
...
SELECT
    ...
    (SELECT COALESCE(SUM(uc.count), 0) FROM usage_counters uc
      WHERE uc.day = ?1 AND uc.metric = 'mail_fetch') AS mail_fetch_today,
    (SELECT COALESCE(SUM(uc.count), 0) FROM usage_counters uc
      WHERE uc.day = ?1 AND uc.metric = 'token_refresh') AS token_refresh_today
`
