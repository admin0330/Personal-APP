// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: mail_accounts.sql

package sqlitedb

import (
	"context"
	"strings"
)

const batchMoveMailAccounts = `-- name: BatchMoveMailAccounts :execrows
UPDATE mail_accounts
SET group_id = ?, updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND deleted_at IS NULL AND id IN (/*SLICE:account_ids*/?)
`

type BatchMoveMailAccountsParams struct {
	GroupID    string
	TenantID   string
	AccountIds []string
}

func (q *Queries) BatchMoveMailAccounts(ctx context.Context, arg BatchMoveMailAccountsParams) (int64, error) {
	query := batchMoveMailAccounts
	var queryParams []interface{}
	queryParams = append(queryParams, arg.GroupID)
	queryParams = append(queryParams, arg.TenantID)
	if len(arg.AccountIds) > 0 {
		for _, v := range arg.AccountIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:account_ids*/?", strings.Repeat(",?", len(arg.AccountIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:account_ids*/?", "NULL", 1)
	}
	result, err := q.db.ExecContext(ctx, query, queryParams...)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const batchSoftDeleteMailAccounts = `-- name: BatchSoftDeleteMailAccounts :execrows
UPDATE mail_accounts
SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP,
    password_enc = '', refresh_token_enc = '', imap_password_enc = ''
WHERE tenant_id = ? AND deleted_at IS NULL AND id IN (/*SLICE:account_ids*/?)
`

type BatchSoftDeleteMailAccountsParams struct {
	TenantID   string
	AccountIds []string
}

func (q *Queries) BatchSoftDeleteMailAccounts(ctx context.Context, arg BatchSoftDeleteMailAccountsParams) (int64, error) {
	query := batchSoftDeleteMailAccounts
	var queryParams []interface{}
	queryParams = append(queryParams, arg.TenantID)
	if len(arg.AccountIds) > 0 {
		for _, v := range arg.AccountIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:account_ids*/?", strings.Repeat(",?", len(arg.AccountIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:account_ids*/?", "NULL", 1)
	}
	result, err := q.db.ExecContext(ctx, query, queryParams...)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const batchUpdateMailAccountProxy = `-- name: BatchUpdateMailAccountProxy :execrows
UPDATE mail_accounts
SET proxy_url = ?, fallback_proxy_url_1 = ?, fallback_proxy_url_2 = ?,
    updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND deleted_at IS NULL AND id IN (/*SLICE:account_ids*/?)
`

type BatchUpdateMailAccountProxyParams struct {
	ProxyUrl          string
	FallbackProxyUrl1 string
	FallbackProxyUrl2 string
	TenantID          string
	AccountIds        []string
}

func (q *Queries) BatchUpdateMailAccountProxy(ctx context.Context, arg BatchUpdateMailAccountProxyParams) (int64, error) {
	query := batchUpdateMailAccountProxy
	var queryParams []interface{}
	queryParams = append(queryParams, arg.ProxyUrl)
	queryParams = append(queryParams, arg.FallbackProxyUrl1)
	queryParams = append(queryParams, arg.FallbackProxyUrl2)
	queryParams = append(queryParams, arg.TenantID)
	if len(arg.AccountIds) > 0 {
		for _, v := range arg.AccountIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:account_ids*/?", strings.Repeat(",?", len(arg.AccountIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:account_ids*/?", "NULL", 1)
	}
	result, err := q.db.ExecContext(ctx, query, queryParams...)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const batchUpdateMailAccountStatus = `-- name: BatchUpdateMailAccountStatus :execrows
UPDATE mail_accounts
SET status = ?, updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND deleted_at IS NULL AND id IN (/*SLICE:account_ids*/?)
`

type BatchUpdateMailAccountStatusParams struct {
	Status     string
	TenantID   string
	AccountIds []string
}

func (q *Queries) BatchUpdateMailAccountStatus(ctx context.Context, arg BatchUpdateMailAccountStatusParams) (int64, error) {
	query := batchUpdateMailAccountStatus
	var queryParams []interface{}
	queryParams = append(queryParams, arg.Status)
	queryParams = append(queryParams, arg.TenantID)
	if len(arg.AccountIds) > 0 {
		for _, v := range arg.AccountIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:account_ids*/?", strings.Repeat(",?", len(arg.AccountIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:account_ids*/?", "NULL", 1)
	}
	result, err := q.db.ExecContext(ctx, query, queryParams...)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const countMailAccounts = `-- name: CountMailAccounts :one
SELECT COUNT(*) FROM mail_accounts WHERE tenant_id = ? AND deleted_at IS NULL
`

func (q *Queries) CountMailAccounts(ctx context.Context, tenantID string) (int64, error) {
	row := q.db.QueryRowContext(ctx, countMailAccounts, tenantID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const countMailAccountsFiltered = `-- name: CountMailAccountsFiltered :one
SELECT COUNT(*) FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
`

type CountMailAccountsFilteredParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
}

// Optional filters use the "NULL means ignore" pattern. tenant_id and
// deleted_at are constant conditions and come first so the composite indexes
// can still help.
//
// Sorting is spelled out as one query per (sort, direction) pair because sqlc
// cannot parameterise ORDER BY at all -- sqlc.arg() inside ORDER BY is left as
// literal text in the emitted SQL, and a bare ? there is silently dropped.
// Both failures only surface at runtime. The service maps the API's
// sort/order pair onto these; the shared WHERE clause is identical in every
// variant, and parity tests cover the pairs.
func (q *Queries) CountMailAccountsFiltered(ctx context.Context, arg CountMailAccountsFilteredParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, countMailAccountsFiltered,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
	)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createMailAccount = `-- name: CreateMailAccount :exec


INSERT INTO mail_accounts (
    id, tenant_id, group_id, email, email_normalized, provider, account_type,
    password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc,
    status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type CreateMailAccountParams struct {
	ID                string
	TenantID          string
	GroupID           string
	Email             string
	EmailNormalized   string
	Provider          string
	AccountType       string
	PasswordEnc       string
	ClientID          string
	RefreshTokenEnc   string
	ImapHost          string
	ImapPort          int64
	ImapPasswordEnc   string
	Status            string
	Remark            string
	SortOrder         int64
	ProxyUrl          string
	FallbackProxyUrl1 string
	FallbackProxyUrl2 string
}

// NOTE: keep this file ASCII-only. sqlc miscomputes query boundaries when a
// .sql file contains multi-byte characters and silently truncates the SQL.
// Explanations live in pkg/repo/accounts.go instead.
//
// Variable-length IN lists use sqlc.slice(), which sqlc expands to IN (?, ?, ?) at call time. The two dialects deliberately
// diverge -- repo method signatures stay identical and pkg/repo/parity_test.go
// guards against drift.
func (q *Queries) CreateMailAccount(ctx context.Context, arg CreateMailAccountParams) error {
	_, err := q.db.ExecContext(ctx, createMailAccount,
		arg.ID,
		arg.TenantID,
		arg.GroupID,
		arg.Email,
		arg.EmailNormalized,
		arg.Provider,
		arg.AccountType,
		arg.PasswordEnc,
		arg.ClientID,
		arg.RefreshTokenEnc,
		arg.ImapHost,
		arg.ImapPort,
		arg.ImapPasswordEnc,
		arg.Status,
		arg.Remark,
		arg.SortOrder,
		arg.ProxyUrl,
		arg.FallbackProxyUrl1,
		arg.FallbackProxyUrl2,
	)
	return err
}

const getMailAccount = `-- name: GetMailAccount :one
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL LIMIT 1
`

type GetMailAccountParams struct {
	TenantID string
	ID       string
}

func (q *Queries) GetMailAccount(ctx context.Context, arg GetMailAccountParams) (MailAccount, error) {
	row := q.db.QueryRowContext(ctx, getMailAccount, arg.TenantID, arg.ID)
	var i MailAccount
	err := row.Scan(
		&i.ID,
		&i.TenantID,
		&i.GroupID,
		&i.Email,
		&i.EmailNormalized,
		&i.Provider,
		&i.AccountType,
		&i.AuthChannel,
		&i.PasswordEnc,
		&i.ClientID,
		&i.RefreshTokenEnc,
		&i.ImapHost,
		&i.ImapPort,
		&i.ImapPasswordEnc,
		&i.Status,
		&i.Remark,
		&i.SortOrder,
		&i.ProxyUrl,
		&i.FallbackProxyUrl1,
		&i.FallbackProxyUrl2,
		&i.LastRefreshAt,
		&i.LastRefreshStatus,
		&i.LastRefreshError,
		&i.RefreshTokenUpdatedAt,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.DeletedAt,
		&i.LastRefreshErrorKind,
	)
	return i, err
}

const getMailAccountByEmail = `-- name: GetMailAccountByEmail :one
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ? AND email_normalized = ? AND deleted_at IS NULL LIMIT 1
`

type GetMailAccountByEmailParams struct {
	TenantID        string
	EmailNormalized string
}

func (q *Queries) GetMailAccountByEmail(ctx context.Context, arg GetMailAccountByEmailParams) (MailAccount, error) {
	row := q.db.QueryRowContext(ctx, getMailAccountByEmail, arg.TenantID, arg.EmailNormalized)
	var i MailAccount
	err := row.Scan(
		&i.ID,
		&i.TenantID,
		&i.GroupID,
		&i.Email,
		&i.EmailNormalized,
		&i.Provider,
		&i.AccountType,
		&i.AuthChannel,
		&i.PasswordEnc,
		&i.ClientID,
		&i.RefreshTokenEnc,
		&i.ImapHost,
		&i.ImapPort,
		&i.ImapPasswordEnc,
		&i.Status,
		&i.Remark,
		&i.SortOrder,
		&i.ProxyUrl,
		&i.FallbackProxyUrl1,
		&i.FallbackProxyUrl2,
		&i.LastRefreshAt,
		&i.LastRefreshStatus,
		&i.LastRefreshError,
		&i.RefreshTokenUpdatedAt,
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.DeletedAt,
		&i.LastRefreshErrorKind,
	)
	return i, err
}

const listMailAccountsByIDs = `-- name: ListMailAccountsByIDs :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ? AND deleted_at IS NULL
  AND id IN (/*SLICE:account_ids*/?)
ORDER BY sort_order, created_at DESC
`

type ListMailAccountsByIDsParams struct {
	TenantID   string
	AccountIds []string
}

func (q *Queries) ListMailAccountsByIDs(ctx context.Context, arg ListMailAccountsByIDsParams) ([]MailAccount, error) {
	query := listMailAccountsByIDs
	var queryParams []interface{}
	queryParams = append(queryParams, arg.TenantID)
	if len(arg.AccountIds) > 0 {
		for _, v := range arg.AccountIds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:account_ids*/?", strings.Repeat(",?", len(arg.AccountIds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:account_ids*/?", "NULL", 1)
	}
	rows, err := q.db.QueryContext(ctx, query, queryParams...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageByCreatedAtAsc = `-- name: ListMailAccountsPageByCreatedAtAsc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY created_at ASC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageByCreatedAtAscParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageByCreatedAtAsc(ctx context.Context, arg ListMailAccountsPageByCreatedAtAscParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageByCreatedAtAsc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageByCreatedAtDesc = `-- name: ListMailAccountsPageByCreatedAtDesc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY created_at DESC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageByCreatedAtDescParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageByCreatedAtDesc(ctx context.Context, arg ListMailAccountsPageByCreatedAtDescParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageByCreatedAtDesc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageByEmailAsc = `-- name: ListMailAccountsPageByEmailAsc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY email_normalized ASC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageByEmailAscParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageByEmailAsc(ctx context.Context, arg ListMailAccountsPageByEmailAscParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageByEmailAsc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageByEmailDesc = `-- name: ListMailAccountsPageByEmailDesc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY email_normalized DESC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageByEmailDescParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageByEmailDesc(ctx context.Context, arg ListMailAccountsPageByEmailDescParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageByEmailDesc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageByLastRefreshAtAsc = `-- name: ListMailAccountsPageByLastRefreshAtAsc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY last_refresh_at ASC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageByLastRefreshAtAscParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageByLastRefreshAtAsc(ctx context.Context, arg ListMailAccountsPageByLastRefreshAtAscParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageByLastRefreshAtAsc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageByLastRefreshAtDesc = `-- name: ListMailAccountsPageByLastRefreshAtDesc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY last_refresh_at DESC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageByLastRefreshAtDescParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageByLastRefreshAtDesc(ctx context.Context, arg ListMailAccountsPageByLastRefreshAtDescParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageByLastRefreshAtDesc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageBySortOrderAsc = `-- name: ListMailAccountsPageBySortOrderAsc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY sort_order ASC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageBySortOrderAscParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageBySortOrderAsc(ctx context.Context, arg ListMailAccountsPageBySortOrderAscParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageBySortOrderAsc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listMailAccountsPageBySortOrderDesc = `-- name: ListMailAccountsPageBySortOrderDesc :many
SELECT id, tenant_id, group_id, email, email_normalized, provider, account_type, auth_channel, password_enc, client_id, refresh_token_enc, imap_host, imap_port, imap_password_enc, status, remark, sort_order, proxy_url, fallback_proxy_url_1, fallback_proxy_url_2, last_refresh_at, last_refresh_status, last_refresh_error, refresh_token_updated_at, created_at, updated_at, deleted_at, last_refresh_error_kind FROM mail_accounts
WHERE tenant_id = ?1
  AND deleted_at IS NULL
  AND (?2 IS NULL OR status = ?2)
  AND (?3 IS NULL OR last_refresh_status = ?3)
  AND (?4 IS NULL OR provider = ?4)
  AND (?5 IS NULL OR group_id = ?5)
  AND (?6 IS NULL
       OR instr(email_normalized, ?6) > 0
       OR instr(lower(remark), ?6) > 0)
ORDER BY sort_order DESC, id
LIMIT ?8 OFFSET ?7
`

type ListMailAccountsPageBySortOrderDescParams struct {
	TenantID      string
	Status        interface{}
	RefreshStatus interface{}
	Provider      interface{}
	GroupID       interface{}
	Q             interface{}
	RowOffset     int64
	RowLimit      int64
}

func (q *Queries) ListMailAccountsPageBySortOrderDesc(ctx context.Context, arg ListMailAccountsPageBySortOrderDescParams) ([]MailAccount, error) {
	rows, err := q.db.QueryContext(ctx, listMailAccountsPageBySortOrderDesc,
		arg.TenantID,
		arg.Status,
		arg.RefreshStatus,
		arg.Provider,
		arg.GroupID,
		arg.Q,
		arg.RowOffset,
		arg.RowLimit,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MailAccount{}
	for rows.Next() {
		var i MailAccount
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.GroupID,
			&i.Email,
			&i.EmailNormalized,
			&i.Provider,
			&i.AccountType,
			&i.AuthChannel,
			&i.PasswordEnc,
			&i.ClientID,
			&i.RefreshTokenEnc,
			&i.ImapHost,
			&i.ImapPort,
			&i.ImapPasswordEnc,
			&i.Status,
			&i.Remark,
			&i.SortOrder,
			&i.ProxyUrl,
			&i.FallbackProxyUrl1,
			&i.FallbackProxyUrl2,
			&i.LastRefreshAt,
			&i.LastRefreshStatus,
			&i.LastRefreshError,
			&i.RefreshTokenUpdatedAt,
			&i.CreatedAt,
			&i.UpdatedAt,
			&i.DeletedAt,
			&i.LastRefreshErrorKind,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const softDeleteMailAccount = `-- name: SoftDeleteMailAccount :execrows
UPDATE mail_accounts
SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP,
    password_enc = '', refresh_token_enc = '', imap_password_enc = ''
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type SoftDeleteMailAccountParams struct {
	TenantID string
	ID       string
}

// Soft delete clears the three credential columns in the same statement.
// Credential ciphertext must not linger behind a deleted_at flag.
func (q *Queries) SoftDeleteMailAccount(ctx context.Context, arg SoftDeleteMailAccountParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, softDeleteMailAccount, arg.TenantID, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const softDeleteMailAccountsByTenant = `-- name: SoftDeleteMailAccountsByTenant :execrows
UPDATE mail_accounts
SET deleted_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP,
    password_enc = '', refresh_token_enc = '', imap_password_enc = ''
WHERE tenant_id = ? AND deleted_at IS NULL
`

// Used when an admin deletes a user: wipes the whole tenant's mailboxes and
// clears the credential ciphertext in the same statement, so nothing sensitive
// lingers behind a deleted_at flag (08 doc section 6, item 6).
func (q *Queries) SoftDeleteMailAccountsByTenant(ctx context.Context, tenantID string) (int64, error) {
	result, err := q.db.ExecContext(ctx, softDeleteMailAccountsByTenant, tenantID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateMailAccount = `-- name: UpdateMailAccount :execrows
UPDATE mail_accounts
SET group_id = ?,
    provider = ?,
    account_type = ?,
    password_enc = ?,
    client_id = ?,
    refresh_token_enc = ?,
    imap_host = ?,
    imap_port = ?,
    imap_password_enc = ?,
    status = ?,
    remark = ?,
    proxy_url = ?,
    fallback_proxy_url_1 = ?,
    fallback_proxy_url_2 = ?,
    updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type UpdateMailAccountParams struct {
	GroupID           string
	Provider          string
	AccountType       string
	PasswordEnc       string
	ClientID          string
	RefreshTokenEnc   string
	ImapHost          string
	ImapPort          int64
	ImapPasswordEnc   string
	Status            string
	Remark            string
	ProxyUrl          string
	FallbackProxyUrl1 string
	FallbackProxyUrl2 string
	TenantID          string
	ID                string
}

func (q *Queries) UpdateMailAccount(ctx context.Context, arg UpdateMailAccountParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateMailAccount,
		arg.GroupID,
		arg.Provider,
		arg.AccountType,
		arg.PasswordEnc,
		arg.ClientID,
		arg.RefreshTokenEnc,
		arg.ImapHost,
		arg.ImapPort,
		arg.ImapPasswordEnc,
		arg.Status,
		arg.Remark,
		arg.ProxyUrl,
		arg.FallbackProxyUrl1,
		arg.FallbackProxyUrl2,
		arg.TenantID,
		arg.ID,
	)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateMailAccountAuthChannel = `-- name: UpdateMailAccountAuthChannel :execrows
UPDATE mail_accounts
SET auth_channel = ?, updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type UpdateMailAccountAuthChannelParams struct {
	AuthChannel string
	TenantID    string
	ID          string
}

// Hot path: clicking a group in the left pane. Skips every optional clause so
// idx_mail_accounts_group applies cleanly.
func (q *Queries) UpdateMailAccountAuthChannel(ctx context.Context, arg UpdateMailAccountAuthChannelParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateMailAccountAuthChannel, arg.AuthChannel, arg.TenantID, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateMailAccountAuthorization = `-- name: UpdateMailAccountAuthorization :execrows
UPDATE mail_accounts
SET client_id = ?, refresh_token_enc = ?, auth_channel = ?,
    refresh_token_updated_at = CURRENT_TIMESTAMP,
    last_refresh_at = CURRENT_TIMESTAMP,
    last_refresh_status = 'success',
    last_refresh_error = '',
    last_refresh_error_kind = '',
    updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type UpdateMailAccountAuthorizationParams struct {
	ClientID        string
	RefreshTokenEnc string
	AuthChannel     string
	TenantID        string
	ID              string
}

// Reauthorization changes credentials only after the new token is verified.
// It must not overwrite group, proxy, remark, or concurrent account edits.
func (q *Queries) UpdateMailAccountAuthorization(ctx context.Context, arg UpdateMailAccountAuthorizationParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateMailAccountAuthorization,
		arg.ClientID,
		arg.RefreshTokenEnc,
		arg.AuthChannel,
		arg.TenantID,
		arg.ID,
	)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateMailAccountRefreshResult = `-- name: UpdateMailAccountRefreshResult :execrows
UPDATE mail_accounts
SET last_refresh_at = CURRENT_TIMESTAMP,
    last_refresh_status = ?,
    last_refresh_error = ?,
    last_refresh_error_kind = ?,
    updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type UpdateMailAccountRefreshResultParams struct {
	LastRefreshStatus    string
	LastRefreshError     string
	LastRefreshErrorKind string
	TenantID             string
	ID                   string
}

func (q *Queries) UpdateMailAccountRefreshResult(ctx context.Context, arg UpdateMailAccountRefreshResultParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateMailAccountRefreshResult,
		arg.LastRefreshStatus,
		arg.LastRefreshError,
		arg.LastRefreshErrorKind,
		arg.TenantID,
		arg.ID,
	)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateMailAccountRefreshToken = `-- name: UpdateMailAccountRefreshToken :execrows
UPDATE mail_accounts
SET refresh_token_enc = ?,
    refresh_token_updated_at = CURRENT_TIMESTAMP,
    updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type UpdateMailAccountRefreshTokenParams struct {
	RefreshTokenEnc string
	TenantID        string
	ID              string
}

// Microsoft rotates refresh tokens. Missing a rotation breaks the account on
// the next refresh, so this must be its own statement that cannot be skipped.
func (q *Queries) UpdateMailAccountRefreshToken(ctx context.Context, arg UpdateMailAccountRefreshTokenParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateMailAccountRefreshToken, arg.RefreshTokenEnc, arg.TenantID, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateMailAccountSort = `-- name: UpdateMailAccountSort :execrows
UPDATE mail_accounts
SET sort_order = ?, updated_at = CURRENT_TIMESTAMP
WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL
`

type UpdateMailAccountSortParams struct {
	SortOrder int64
	TenantID  string
	ID        string
}

// Drag-and-drop reordering. Narrow UPDATE on purpose: it must not touch group,
// remark, proxy or any credential column -- the protocol layer rewrites those
// on its own schedule and a full-row rewrite would clobber them (see
// UpdateMailAccountAuthChannel). execrows is what turns a cross-tenant ID into
// ErrNotFound: the WHERE carries tenant_id, so a foreign account updates 0 rows.
func (q *Queries) UpdateMailAccountSort(ctx context.Context, arg UpdateMailAccountSortParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateMailAccountSort, arg.SortOrder, arg.TenantID, arg.ID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}
