...
SELECT id, tenant_id, code_hash, created_by, expires_at, redeemed_at, created_at FROM device_codes WHERE code_hash = ? LIMIT 1
`
...
SELECT id, tenant_id, token_hash, token_hint, label, created_by, expires_at, revoked_at, created_at FROM device_tokens WHERE tenant_id = ? ORDER BY created_at DESC
`
