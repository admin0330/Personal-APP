...
func (h *AuthHandler) RedeemInvite(c *echo.Context) error {
	var req model.InviteRegisterRequest
	if e := c.Bind(&req); e != nil {
		return failure(c, http.StatusBadRequest, e)
	}
	v, token, e := h.service.RedeemInvite(c.Request().Context(), req)
	if e != nil {
		return failure(c, http.StatusBadRequest, e)
	}
	h.setCookie(c, token)
	return success(c, v, "账号创建成功")
}
